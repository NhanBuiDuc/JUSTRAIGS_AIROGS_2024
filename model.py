        ...
        *,
        ...
        for ind in range(depth):
            ...
            layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim=dim, dim_head=dim_head,
                        heads=heads, dropout=attn_dropout)),
                PreNorm(dim, FeedForward(dim=dim, dropout=ff_dropout)),
                PerformerAttention(dim=dim, heads=heads,
                                   dim_head=dim_head) if should_pool else None
            ]))
    ...
    def forward(self, x, mask=None):
        ...
        for attn, ff, efficient_attn in self.layers:
            ...
            if exists(efficient_attn):
                ...
                pool_attn_mask = None
                if exists(mask):
                    pool_attn_mask = repeat(
                        mask, 'b n -> b (n l)', l=num_layers)

                attended_tokens = efficient_attn(
                    all_tokens, mask=pool_attn_mask)

                attended_tokens = rearrange(attended_tokens, 'b n c -> b c n')
                pooled_tokens = F.max_pool1d(
                    attended_tokens, kernel_size=num_layers, stride=num_layers)
    ...
    def __init__(
        ...
        *,
        ...
        for ind in range(depth):
            ...
            layers.append(nn.ModuleList([
                PreNorm(dim, Attention(causal=True, dim=dim,
    ...
    def forward(self, x, mask=None):
        ...
        for ind, (attn, ff, layer_axial_attn) in enumerate(self.layers):
            ...
            if exists(layer_axial_attn):
                ...
                layer_tokens = rearrange(torch.stack(
                    layers_to_pool), 'l b n d -> (b n) l d')

                attended_tokens = layer_axial_attn(layer_tokens)
                attended_tokens = rearrange(
                    attended_tokens, '(b n) l d -> b n l d', b=b)
